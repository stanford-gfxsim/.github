# Code and Data Releases

Research code and datasets from publications by Doug L. James' research group and collaborators.

Group website: https://graphics.stanford.edu/~djames/group  
Full publication list: https://graphics.stanford.edu/~djames/publications  
Contact: djames@cs.stanford.edu

## Stanford

**Mixwell: Sharp 2D Fluid Brushes for Progressive Physics-Based Mixing** (SIGGRAPH 2026)
Doug L. James, Ethan James
- Code: https://github.com/dougjam/mixwell-2026 (GLSL/HLSL reference implementations, Graphics Replicability Stamp)
- Interactive demos: https://dougjam.github.io/mixwell-2026

**Acoustic Reliefs** (SIGGRAPH Asia 2025)
Jeremy Chew, Michal Piovarci, Kangrui Xue, Doug L. James, Bernd Bickel
- Code: https://github.com/mickey1356/acoustic_reliefs
- Project: https://cdl.ethz.ch/publications/acoustic-reliefs

**WaveBlender: Practical Sound-Source Animation in Blended Domains** (SIGGRAPH Asia 2024)
Kangrui Xue, Jui-Hsien Wang, Timothy R. Langlois, Doug L. James
- Code: https://github.com/kangruix/WaveBlender
- Dataset: https://graphics.stanford.edu/papers/waveblender/dataset

**Improved Water Sound Synthesis Using Coupled Bubbles** (SIGGRAPH 2023)
Kangrui Xue, Ryan M. Aronson, Jui-Hsien Wang, Timothy R. Langlois, Doug L. James
- Code: https://github.com/kangruix/FluidSound

**RealImpact: A Dataset of Impact Sound Fields for Real Objects** (CVPR 2023, Highlight)
Samuel Clarke, Ruohan Gao, Mason Wang, Mark Rau, Julia Xu, Jui-Hsien Wang, Doug L. James, Jiajun Wu
- Code: https://github.com/samuel-clarke/RealImpact
- Dataset: https://samuelpclarke.com/realimpact

**svMorph: Interactive Geometry-Editing Tools for Virtual Patient-Specific Vascular Anatomies** (J Biomech Eng 2023)
Jonathan Pham, Sofia Wyetzner, Martin R. Pfaller, David W. Parker, Doug L. James, Alison L. Marsden
- Code: https://github.com/SimVascular/svMorph

**DiffImpact: Differentiable Rendering and Identification of Impact Sounds** (CoRL 2021)
Samuel Clarke, Negin Heravi, Mark Rau, Ruohan Gao, Jiajun Wu, Doug James, Jeannette Bohg
- Code: https://github.com/samuel-clarke/diffimpact

**Fast Linking Numbers for Topology Verification of Loopy Structures** (SIGGRAPH 2021)
Ante Qu, Doug L. James
- Code: https://github.com/antequ/fastlinkingnumbers
- Dataset: https://graphics.stanford.edu/papers/fastlinkingnumbers

**Personalized HRTF Modeling Using DNN-Augmented BEM** (ICASSP 2021)
Mengfan Zhang, Jui-Hsien Wang, Doug L. James
- Data (head meshes, simulated HRTFs): https://ccrma.stanford.edu/~zhangmf/paper/ICASSP2021

**KleinPAT: Optimal Mode Conflation for Time-Domain Precomputation of Acoustic Transfer** (SIGGRAPH 2019)
Jui-Hsien Wang, Doug L. James
- Code: https://github.com/jhwang7628/openpbso (openpbso, an open-source library for physics-based sound)
- Dataset: https://graphics.stanford.edu/projects/kleinpat

**Toward Wave-based Sound Synthesis for Computer Animation** (SIGGRAPH 2018)
Jui-Hsien Wang, Ante Qu, Timothy R. Langlois, Doug L. James
- Code: https://github.com/jhwang7628/acoustics (FDTD acoustic wavesolver codebase)
- Dataset: https://graphics.stanford.edu/projects/wavesolver

**Interactive Design of Periodic Yarn-Level Cloth Patterns** (SIGGRAPH Asia 2018)
Jonathan Leaf, Rundong Wu, Eston Schweickart, Doug L. James, Steve Marschner
- Dataset (yarn-level pattern corpus): https://graphics.stanford.edu/projects/yarnsim

## Pixar collaborations

**Phong Deformation: A Better C0 Interpolant for Embedded Deformation** (SIGGRAPH 2020)
Doug L. James
- Code: https://www.dropbox.com/scl/fi/77z7d3jtcsp9ikkotou5j/PhongDefoCode_v0.zip?rlkey=9rn6yy7mlp71sbnfosyyyoq2g&dl=0
- Paper: https://research.pixar.com/docs/2020.SiggraphPapers.J.pdf

**Sharp Kelvinlets: Elastic Deformations with Cusps and Localized Falloffs** (DigiPro 2019)
Fernando de Goes, Doug L. James
- Code: https://research.pixar.com/docs/2019.Others.GJ_suppl.zip
- Paper: https://research.pixar.com/docs/2019.Others.GJ.pdf

**Dynamic Kelvinlets: Secondary Motions based on Fundamental Solutions of Elastodynamics** (SIGGRAPH 2018)
Fernando de Goes, Doug L. James
- Code: https://research.pixar.com/docs/2018.SiggraphPapers.GJ_suppl.zip
- Paper: https://research.pixar.com/docs/2018.SiggraphPapers.GJ.pdf

**Regularized Kelvinlets: Sculpting Brushes based on Fundamental Solutions of Elasticity** (SIGGRAPH 2017)
Fernando de Goes, Doug L. James
- Closed-form solutions in the paper: https://research.pixar.com/docs/2017.SiggraphPapers.GJ.pdf
- Open-source implementation: `igl::kelvinlets` in libigl (https://libigl.github.io/dox/kelvinlets_8h.html)

## Cornell

**Eigenmode Compression for Modal Sound Models** (SIGGRAPH 2014)
Timothy R. Langlois, Steven S. An, Kelvin K. Jin, Doug L. James
- Code + data: http://www.cs.cornell.edu/projects/Sound/modec/modec.zip

**Inverse-Foley Animation: Synchronizing Rigid-Body Motions to Sound** (SIGGRAPH 2014)
Timothy R. Langlois, Doug L. James
- Code + data: http://www.cs.cornell.edu/projects/Sound/ifa/ifa.zip

**Energy-based Self-Collision Culling for Arbitrary Mesh Deformations** (SIGGRAPH 2012)
Changxi Zheng, Doug L. James
- Benchmark datasets: http://www.cs.cornell.edu/projects/escc

**Stitch Meshes for Modeling Knitted Clothing with Yarn-level Detail** (SIGGRAPH 2012)
Cem Yuksel, Jonathan M. Kaldor, Doug L. James, Steve Marschner
- Yarn-level cloth models (BCC format): http://www.cemyuksel.com/research/yarnmodels

**Physics-Based Character Skinning Using Multi-Domain Subspace Deformations** (SCA 2011, Best Paper; IEEE TVCG 2012)
Theodore Kim, Doug L. James
- Code: Cubica subspace-deformation toolkit, https://www.tkim.graphics/cubica

**Animating Fire with Sound** (SIGGRAPH 2011)
Jeffrey N. Chadwick, Doug L. James
- Code (bandwidth extension, sound texture synthesis): http://www.cs.cornell.edu/projects/Sound/fire

**Skipping Steps in Deformable Simulation with Online Model Reduction** (SIGGRAPH Asia 2009)
Theodore Kim, Doug L. James
- Code: Cubica subspace-deformation toolkit, https://www.tkim.graphics/cubica

**Harmonic Fluids** (SIGGRAPH 2009)
Changxi Zheng, Doug L. James
- Code: http://www.cs.cornell.edu/projects/HarmonicFluids/code/Default.htm

**Optimizing Cubature for Efficient Integration of Subspace Deformations** (SIGGRAPH Asia 2008)
Steven An, Theodore Kim, Doug L. James
- Code: http://www.cs.cornell.edu/~stevenan/cubature/cubacode.tar
- Also implemented in the Cubica toolkit: https://www.tkim.graphics/cubica

**Wavelet Turbulence for Fluid Simulation** (SIGGRAPH 2008)
Theodore Kim, Nils Thuerey, Doug L. James, Markus Gross
- Code: http://www.cs.cornell.edu/~tedkim/WTURB/source.html

**FastLSM: Fast Lattice Shape Matching for Robust Real-Time Deformation** (SIGGRAPH 2007)
Alec R. Rivers, Doug L. James
- Code (FastLSM library with source, demos): http://www.alecrivers.com/fastlsm


## CMU

**Precomputed Acoustic Transfer** (SIGGRAPH 2006)
Doug L. James, Jernej Barbic, Dinesh K. Pai
- Code (multipole/dipole transfer evaluation) and real-time demos: http://graphics.cs.cmu.edu/projects/pat

**Real-Time Subspace Integration of St.Venant-Kirchhoff Deformable Models** (SIGGRAPH 2005)
Jernej Barbic, Doug L. James
- Code: Vega FEM library implements this method [BJ05], including the Large Modal Deformation Factory precomputation utility: https://viterbi-web.usc.edu/~jbarbic/vega
- Demos: http://graphics.cs.cmu.edu/projects/stvk

**Skinning Mesh Animations** (SIGGRAPH 2005)
Doug L. James, Christopher D. Twigg
- Demos + data: http://graphics.cs.cmu.edu/projects/sma

**Mesh Ensemble Motion Graphs** (ACM TOG 2007)
Doug L. James, Christopher D. Twigg, Andrew Cove, Robert Y. Wang
- Data + viewer with source: http://graphics.cs.cmu.edu/projects/memg/data

**Many-Worlds Browsing for Control of Multibody Dynamics** (SIGGRAPH 2007)
Christopher D. Twigg, Doug L. James
- Demo: http://graphics.cs.cmu.edu/projects/mwb/demo